...
@echo ***************************************************************
%WINDIR%\Microsoft.NET\Framework\v4.0.30319\msbuild  %projectpath%\%projectname%.csproj /verbosity:normal /fl /t:Rebuild /p:Configuration=%config%,OutputPath=bin\%config%\ /property:GenerateLibraryLayout=false /p:NoWarn=0618
...
@echo *******************************************
REM %nuget% pack "%projectpath%\%projectname%.nuspec" -symbols -o NuGet\%projectname% -p Configuration=%config% %version%
%nuget% pack NuGet\%projectname%\%projectname%.nuspec -o NuGet\%projectname%
